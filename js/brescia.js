...
$(document).ready(function() {
  ...
  $('[data-toggle="offcanvas"]').on('click', function () {
	var $toggler = $( this );
	var $togglerstate = $toggler.attr('aria-expanded');
	  
	  
	if ( $togglerstate === 'false' ) {
...
if ($(window).width() > 991) {
	...
	$(window).scroll(function() {
		var $body = $("body");
		var $top = 220;
		var $tertnavs = $("nav.tertNav ul");
		var $tertnavas = $("nav.tertNav ul button, nav.tertNav ul a");
		if ($(window).scrollTop() >= $top) {
			$body.addClass('rolled');
			$tertnavs.attr("aria-hidden", true);
			$tertnavas.attr("tabindex", -1);
		}
		else
		{
				$body.removeClass('rolled');
				$tertnavs.attr("aria-hidden", false);
				$tertnavas.attr("tabindex", 0);
		}
	});	
...
$('button.searchTrigger ').on( "click keypress", function(ev) {
	
	if(confirmClick(ev) === true){
		ev.preventDefault(); // needed for firefox and chrome enter
		if (!$('body').hasClass( "searchOpen" )){ // simple toggle conflicting with focusout
			$('body').addClass( "searchOpen" );
			$('.mainSearch input').attr("tabindex", 0);
			$('form.mainSearch').attr('aria-hidden', 'false');
			$('.mainSearch input').focus();
		}
		else if ( $('body').hasClass( "searchOpen" ) )  { // simple toggle conflicting with focusout
			$('body').removeClass( "searchOpen" );
			$('form.mainSearch').attr('aria-hidden', 'true');
		}
	}
});		
	...
	$( '.mainSearch input' ).focusout(function() {
	 window.setTimeout(function() {   // preventing conflict of focusout and click if focusout is caused by click
		if ($('body').hasClass( "searchOpen" )){
			$('body').removeClass( "searchOpen" );
			$('form.mainSearch').attr('aria-hidden', 'true');
			$('.mainSearch input').attr("tabindex", -1);
		}
	 }, 200);   // preventing conflict of focusout and click if focusout is caused by click
});	
	...
	if ($(window).width() < 992) {
		$('input#sitesearch, form.mainSearch button').attr("tabindex", 0);
		$('form.mainSearch').attr("aria-hidden", false);
	}	
	
// SLIDER
if ( $( ".bresc2slide" ).length ) {	
...
if ( $( "#navbarSide" ).length ) {		
	...
	if ($(window).width() > 991) {
		$('button.navbar-toggler#sideMenu').attr("aria-expanded", true);
		$('div.navbar-collapse#navbarSide').attr("aria-hidden", false);
	}	
	
	
	$('button.navbar-toggler#sideMenu').on( "click keypress", function(ev) {
		if(confirmClick(ev) === true){
			if (!$('button.navbar-toggler#sideMenu').hasClass( "collapsed")){ 
				$('div.navbar-collapse#navbarSide').attr('aria-hidden', 'true');
			}
			else if ( $('button.navbar-toggler#sideMenu').hasClass( "collapsed") )  { 
					$('div.navbar-collapse#navbarSide').attr('aria-hidden', 'false');
			}
		}	
	});		
...
// accessibile overlay link hovers 
	
if ( $( ".overlayLink" ).length ) {

	$( "a.overlayLink" ).hover( function(){
...
if ( $( ".directoryFilter" ).length ) {	
	...
	var directory; //programs
	  $.getJSON('http://clients.stamats.com/brescia/HTML/final2/data/visdir201906.json', function(data) {
	...
	function filterDirectory() {  
		...
		FilterJS(directory, "#dirList", {
		  template: '#template',
		 criterias:[
			{field: 'Type', ele: '#Type input:checkbox'}
	   ],


		search: { ele: '#text_filter' },  //, fields: ['Displayname', 'Type']
...
if ( $( "#type-items" ).length ) {
  ...
  var typed = new Typed('#type-space', {
    stringsElement: '#type-items',
	    typeSpeed: 20,
    backSpeed: 0,
    backDelay: 2000,
    shuffle: true,
...
function tabber(e) {
    if (e.keyCode === 9) { 
        ...
        window.removeEventListener('keydown', tabber);
    }
}

window.addEventListener('keydown', tabber);	
